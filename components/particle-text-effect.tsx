...
class Particle {
  ...
  kill(width: number, height: number) {
    if (!this.isKilled) {
      ...
      // Begin blending color to the theme background
      const isDark = document.documentElement.classList.contains("dark")
      const bgColor = isDark ? { r: 0, g: 0, b: 0 } : { r: 255, g: 255, b: 255 }
      this.startColor = {
...
const DEFAULT_WORDS = ["ZFRED", "REQUEST", "SQL", "TERMINAL"]
const DEFAULT_COLOR = "#2878BE"
const DEFAULT_Y_RATIO = 0.3
...
export function ParticleTextEffect({
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    offscreenCtx.fillText(word, canvas.width / 2, canvas.height * yRatio)
  ...
  const animate = () => {
    ...
    // Background with motion blur (adapts to theme)
    const isDark = document.documentElement.classList.contains("dark")
    ctx.fillStyle = isDark ? "rgba(0, 0, 0, 0.12)" : "rgba(255, 255, 255, 0.45)"
