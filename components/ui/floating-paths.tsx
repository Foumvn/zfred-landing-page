...
function FloatingPaths({ position }: { position: number }) {
  const paths = Array.from({ length: 24 }, (_, i) => {
    const randomDashLength = 60 + ((i * 37) % 80) // 60-140px long dashes
    const randomGap = 150 + ((i * 53) % 100) // 150-250px gaps
    const randomDuration = 8 + ((i * 29) % 12) // 8-20s duration
    const randomDelay = (i * 17) % 10 // 0-10s delay
